...
int main(int argc, char const *argv[]) {
  ...
  // SomeIP source
  auto &sourceSomeIP = SomeIPTelemetrySourceImpl::getInstance();

  // run Client
  sourceSomeIP.openSource();
  ...
  // thread 3: SomeIP Source
  std::thread someIPReader([&]() {
    for (int i = 0; i < 10; i++) {
      std::string rawData;
      if (sourceSomeIP.readSource(rawData)) {
        // format data
        std::unique_ptr<LogFormatter<GpuPolicy>> formatter =
